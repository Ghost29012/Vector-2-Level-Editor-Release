...
public class BuildMapVec2 : MonoBehaviour
{
    ...
    // In editor, XML lives under Assets/. In a standalone build, Assets/ is
    // inside the read-only .app bundle, so we use persistentDataPath instead.
    static string XmlBasePath =>
#if UNITY_EDITOR
    ...
    // Called once to copy XML templates from StreamingAssets to persistentDataPath
    // so the build can read and write them freely.
    static void EnsureXmlReady()
    {
#if !UNITY_EDITOR
        string[] dirs = {
            "/XML", "/XML/dzip", "/XML/dzip/level_xml"
        };
        foreach (var d in dirs)
            Directory.CreateDirectory(Application.persistentDataPath + d);

        // Copy template files from StreamingAssets if not already present
        string[] templates = {
            "/XML/build-map - vec2.xml",
            "/XML/empty-map-DONT-MODIFY-vec2.xml"
        };
        foreach (var t in templates) {
            string dest = Application.persistentDataPath + t;
            string src  = Application.streamingAssetsPath + t;
    ...
    void Awake()
    {
        vectorFilePath = (VectorierSettings.RoomsDirectory ?? "").Trim().TrimEnd('/', '\\');
        EnsureXmlReady();
    }
    ...
    public static void Build(bool useDZ, bool compileMap, bool moveXml)
    {
        ...
        // Always normalise — strip trailing slashes
        buildMap.vectorFilePath = (buildMap.vectorFilePath ?? "").TrimEnd('/', '\\').Trim();
        Debug.Log("Building...");
        ...
        //Erase last build
        File.Delete(XmlBasePath + "/XML/build-map - vec2.xml");
        File.Copy(XmlBasePath + "/XML/empty-map-DONT-MODIFY-vec2.xml", XmlBasePath + "/XML/build-map - vec2.xml");

        //Open the object.xml
        XmlDocument xml = new XmlDocument();
        xml.Load(XmlBasePath + "/XML/build-map - vec2.xml");
        ...
        foreach (XmlNode node in xml.DocumentElement.SelectSingleNode("/Root/Track"))
        {
            ...
            foreach (GameObject imageInScene in imagesInScene)
            {
                ...
                xml.Save(XmlBasePath + "/XML/dzip/level_xml/" + buildMap.mapToOverride + ".xml"); //Apply the modification to the build-map.xml file}
            }
            ...
            foreach (GameObject triggerInScene in GameObject.FindGameObjectsWithTag("Trigger"))
            {
                ...
                if (triggerNode != null) node.AppendChild(triggerNode);
                ...
                using (XmlWriter writer = XmlWriter.Create(XmlBasePath + "/XML/dzip/level_xml/" + buildMap.mapToOverride + ".xml", settings))
                {
                    xml.Save(writer);
                }
            }
            ...
            foreach (GameObject areaInScene in GameObject.FindGameObjectsWithTag("Area"))
            {
                ...
                if (areaNode != null) node.AppendChild(areaNode);
                xml.Save(XmlBasePath + "/XML/dzip/level_xml/" + buildMap.mapToOverride + ".xml"); //Apply the modification to the build-map.xml file}
            }
    ...
    void StartDzip(bool useDZ)
    {
        ...
        string batchFilePath = Path.Combine(XmlBasePath, "XML/dzip", batchFileName);
        ...
        // Route through cmd /c with the path quoted — this correctly handles
        // spaces in any part of batchFilePath on all Windows versions.
        ...
        try
        {
            ...
            else
            {
                ...
                string sourceFilePath = Path.Combine(XmlBasePath, "XML/dzip/level_xml.dz");
    ...
    {
        // Sanitise inputs — strip trailing slashes from dir, strip .xml from filename
        string cleanDir  = (vectorFilePath ?? "").TrimEnd('/', '\\').Trim();
        string cleanName = System.Text.RegularExpressions.Regex
            .Replace(mapToOverride ?? "escape_room", @"\.xml$", "",
                System.Text.RegularExpressions.RegexOptions.IgnoreCase).Trim();

        if (string.IsNullOrEmpty(cleanDir))
        {
            Debug.LogError("[Vectorier] Rooms Directory is not set! Set it in the inspector under Vectorier Settings.");
            return;
        }
        if (string.IsNullOrEmpty(cleanName))
        {
            Debug.LogError("[Vectorier] Map To Override is empty!");
            return;
        }

        string sourceFilePath      = Path.Combine(XmlBasePath, "XML/dzip/level_xml/" + cleanName + ".xml");
        string destinationFilePath = Path.Combine(cleanDir, cleanName + ".xml");

        Debug.Log($"[Vectorier] Moving XML from {sourceFilePath} → {destinationFilePath}");

        if (!File.Exists(sourceFilePath))
        {
            Debug.LogError("[Vectorier] Built XML not found at: " + sourceFilePath);
            return;
        }
        // Destination directory must already exist — we never create it automatically
        // (that's what was causing the phantom 'rooms' subfolder)
        string destDir = Path.GetDirectoryName(destinationFilePath);
        if (!Directory.Exists(destDir))
        {
            Debug.LogError($"[Vectorier] Destination directory does not exist: {destDir}\nCheck your Rooms Directory setting.");
            return;
        }
        if (File.Exists(destinationFilePath))
            File.Delete(destinationFilePath);
        File.Copy(sourceFilePath, destinationFilePath);
        Debug.Log("[Vectorier] XML saved to: " + destinationFilePath);
    }
    ...
    void ConvertToImage(XmlNode node, XmlDocument xml, GameObject imageInScene, out XmlElement imageNode, bool localPosition)
    {
        ...
        if (imageInScene.name != "Camera")
        {
            ...
            if (spriteRenderer != null && spriteRenderer.sprite != null) //Get the Image Size in Width and Height
            {
                ...
                // Displayed size in V2 units — always positive
                float displayW = Math.Abs(width * scale.x);
                float displayH = Math.Abs(height * scale.y);

                // Width/Height = displayed size, NativeX/NativeY = unscaled sprite size.
                // Keeping this relationship is required for runtime parity with imported assets.
                ielement.SetAttribute("Width",  displayW.ToString());
                ielement.SetAttribute("Height", displayH.ToString());
                ...
                // Check the rotation
                {
                    ...
                    float A, B, C, D, Tx, Ty;
                    ConvertToMarmaladeMatrix(imageInScene, displayW, displayH, out A, out B, out C, out D, out Tx, out Ty);
    ...
    {
        // Always use positive dimensions — negative scale must never reach the Matrix
    ...
    void ConvertToObjectReference(XmlNode node, XmlDocument xml, GameObject ObjectReferenceTagInScene, out XmlElement objRefNode, bool localPosition)
    {
        ...
        if (ObjectReferenceTagInScene.name != "Camera")
        {
            ...
            if (Mathf.Abs(ObjectReferenceTagInScene.transform.eulerAngles.z) > Mathf.Epsilon || scale.x != 1 || scale.y != 1)
            {
                ...
                {
                    float A, B, C, D, Tx, Ty;
                    ConvertToMarmaladeMatrix(ObjectReferenceTagInScene, Math.Abs(scale.x), Math.Abs(scale.y), out A, out B, out C, out D, out Tx, out Ty);
    ...
    void ConvertToPlatform(XmlNode node, XmlDocument xml, GameObject platformInScene, out XmlElement platformNode, bool localPosition) // Platform Collision (Invisible block that is collide-able)
    {
        ...
        if (platformInScene.name != "Camera")
        {
            ...
            if (spriteRenderer != null && spriteRenderer.sprite != null) //Get the Sprite Size in Width and Height
            {
                ...
                P_element.SetAttribute("Width", Math.Abs(width * scale.x).ToString()); //Width of the Image
                P_element.SetAttribute("Height", Math.Abs(height * scale.y).ToString()); //Height of the Image
    ...
    void ConvertToSpawn(XmlNode node, XmlDocument xml, GameObject spawnInScene, out XmlElement spawnNode, bool localPosition)
    {
        ...
        if (RespawnComponent != null)
        {
            ...
            if (spriteRenderer != null && spriteRenderer.sprite != null) //Get the Sprite Size in Width and Height
            {
                ...
                // Set the width and height accordingly to the scale in the editor
                triggerElement.SetAttribute("Width", Math.Abs(width * scale.x).ToString()); //Width of the Image
                triggerElement.SetAttribute("Height", Math.Abs(height * scale.y).ToString()); //Height of the Image
            }
    ...
    void ConvertToTrapezoid(XmlNode node, XmlDocument xml, GameObject trapezoidInScene, out XmlElement trapezoidNode, bool localPosition) // Trapezoid Collision (Slope)
    {
        ...
        if (Regex.Replace(trapezoidInScene.name, @" \((.*?)\)", string.Empty) == "trapezoid_type1") // Slope Default
        {
            ...
            if (spriteRenderer != null && spriteRenderer.sprite != null) //Get the Sprite Size in Width and Height
            {
                ...
                // Set the width and height accordingly to the scale in the editor
                T_element.SetAttribute("Width", Math.Abs(width * scale.x).ToString()); //Width of the Trapezoid
                T_element.SetAttribute("Height", (Math.Abs(height * scale.y) + 1).ToString()); //Height1 of the Trapezoid
        ...
        else if (Regex.Replace(trapezoidInScene.name, @" \((.*?)\)", string.Empty) == "trapezoid_type2") // Slope Mirrored
        {
            ...
            if (spriteRenderer != null && spriteRenderer.sprite != null) //Get the Sprite Size in Width and Height
            {
                ...
                // Set the width and height accordingly to the scale in the editor
                T_element.SetAttribute("Width", Math.Abs(width * scale.x).ToString()); //Width of the Trapezoid
                T_element.SetAttribute("Height", (Math.Abs(height * scale.y) + 1).ToString()); //Height of the Trapezoid
        ...
        if (triggerInScene.name != "Camera")
        {
            ...
            if (dynamicTrigger != null)
            {
                ...
                if (spriteRenderer != null && spriteRenderer.sprite != null) //Get the Sprite Size in Width and Height
                {
                    ...
                    T_element.SetAttribute("Width", Math.Abs(width * scale.x).ToString()); //Width of the Image
                    T_element.SetAttribute("Height", Math.Abs(height * scale.y).ToString()); //Height of the Image
                }
    ...
    void ConvertToDynamicTrigger(XmlNode node, XmlDocument xml, GameObject dynamictriggerInScene, out XmlElement dyntriggerNode, bool localPosition)
    {
        ...
        if (dynamicTrigger == null) {
            Debug.LogWarning($"[Vectorier] GameObject '{dynamictriggerInScene.name}' is tagged 'Dynamic Trigger' but has no DynamicTrigger component — skipping.");
            return;
        }
    ...
    void ConvertToArea(XmlNode node, XmlDocument xml, GameObject areaInScene, out XmlElement areaNode, bool localPosition)
    {
        ...
        if (areaInScene.name != "Camera")
        {
            ...
            if (spriteRenderer != null && spriteRenderer.sprite != null) //Get the Sprite Size in Width and Height
            {
                ...
                // Set the width and height accordingly to the scale in the editor
                A_element.SetAttribute("Width", Math.Abs(width * scale.x).ToString()); //Width of the Image
                A_element.SetAttribute("Height", Math.Abs(height * scale.y).ToString()); //Height of the Image
    ...
    void ConvertToDynamic(XmlNode node, XmlDocument xml, GameObject dynamicInScene, UnityEngine.Transform dynamicInSceneTransform)
    {
        ...
        foreach (GameObject imageObject in ImageObjects)
        {
            ...
            if (spriteRenderer != null && spriteRenderer.sprite != null) //Get the Image Size in Width and Height
            {
                ...
                // Set the width and height accordingly to the scale in the editor
                ielement.SetAttribute("Width", Math.Abs(width * scale.x).ToString()); //Width of the Image
                ielement.SetAttribute("Height", Math.Abs(height * scale.y).ToString()); //Height of the Image
                ...
                // Check the rotation
                {
                    // Convert the rotation to the Marmalade transformation matrix
                    float A, B, C, D, Tx, Ty;
                    ConvertToMarmaladeMatrix(imageObject, Math.Abs(width * scale.x), Math.Abs(height * scale.y), out A, out B, out C, out D, out Tx, out Ty);
        ...
        xml.Save(XmlBasePath + "/XML/dzip/level_xml/" + mapToOverride + ".xml"); //Apply the modification to the build-map.xml file}
    }
    void ConvertToObject(XmlNode node, XmlDocument xml, GameObject objectInScene, out XmlElement objectNode, bool localPosition)
    {
        ...
        if (objectInScene.name != "Camera")
        {
            ...
            {
                string ctag = childObject.gameObject.tag;

                // Skip non-exportable tags
                if (ctag == "Untagged" || ctag == "Image" || ctag == "Unused") continue;

                // Unified if/else if chain — no fall-through, no duplicate appends
                if (ctag == "Out")
                {
                    XmlElement n1; ConvertToOut(node, xml, childObject.gameObject, out n1, true);
                    if (n1 != null) contentElement.AppendChild(n1);
                }
                else if (ctag == "In")
                {
                    XmlElement n1; ConvertToIn(node, xml, childObject.gameObject, out n1, true);
                    if (n1 != null) contentElement.AppendChild(n1);
                }
                else if (ctag == "Object")
                {
                    XmlElement n1; ConvertToObject(node, xml, childObject.gameObject, out n1, true);
                    if (n1 != null) contentElement.AppendChild(n1);
                }
                else if (ctag == "Spawn")
                {
                    XmlElement n1; ConvertToSpawn(node, xml, childObject.gameObject, out n1, true);
                    if (n1 != null) contentElement.AppendChild(n1);
                }
                else if (ctag == "Platform")
                {
                    XmlElement n1; ConvertToPlatform(node, xml, childObject.gameObject, out n1, true);
                    if (n1 != null) contentElement.AppendChild(n1);
                }
                else if (ctag == "Trapezoid")
                {
                    XmlElement n1; ConvertToTrapezoid(node, xml, childObject.gameObject, out n1, true);
                    if (n1 != null) contentElement.AppendChild(n1);
                }
                else if (ctag == "Area")
                {
                    XmlElement n1; ConvertToArea(node, xml, childObject.gameObject, out n1, true);
                    if (n1 != null) contentElement.AppendChild(n1);
                }
                else if (ctag == "Trigger")
                {
                    XmlElement n1; ConvertToTrigger(node, xml, childObject.gameObject, out n1, true);
                    if (n1 != null) contentElement.AppendChild(n1);
                }
                else if (ctag == "Dynamic Trigger")
                {
                    XmlElement n1; ConvertToDynamicTrigger(node, xml, childObject.gameObject, out n1, false);
                    if (n1 != null) contentElement.AppendChild(n1);
                }
                else if (ctag == "Object Reference")
                {
                    XmlElement n1; ConvertToObjectReference(node, xml, childObject.gameObject, out n1, true);
                    if (n1 != null) contentElement.AppendChild(n1);
                }
                else if (ctag == "Animation")
                {
                    XmlElement n1; ConvertToAnimation(node, xml, childObject.gameObject, out n1, true);
                    if (n1 != null) contentElement.AppendChild(n1);
                }
                else if (ctag == "Camera")
                {
                    XmlElement n1; ConvertToCamera(node, xml, childObject.gameObject, out n1, true);
                    if (n1 != null) contentElement.AppendChild(n1);
                }
            }

            // Append content ONCE, after the loop — not inside it
